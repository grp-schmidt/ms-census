...
Analyse resulting linear models per marker gene and cutoff (Extended Figure 8).

```{r, fig.width=15, fig.height=15}
#First, get the marker genes to be included per domain
#use.marker_id
#use.marker_id.arch
...
collect.rarefaction %>%
  ...
  filter(
    marker_id %in% c(
      sample(use.marker_id, 10),
      sample(use.marker_id.arch, 10)
    )
  ) %>%
  left_join(dat.hmm %>% select(marker_id, domain), by = "marker_id") %>%
  ggplot(aes(x = n.cluster, y = n.species, colour = source, fill = source, group = source)) +
    geom_abline(slope = 1, colour = "grey") +
    geom_smooth(aes(group = source), method = "lm", formula = "y ~ x + 0", se = F, linewidth = 0.7) +
    geom_point(shape = 21, colour = "grey", stroke = 0.1, size = 1) +
    scale_x_sqrt() +
    scale_y_sqrt() +
    scale_colour_brewer(palette = "Paired") +
    scale_fill_brewer(palette = "Paired") +
    xlab("n of sequence clusters (at 96.5%)") +
    ylab("n of species discovered") +
    facet_wrap(
      domain + marker_id ~ .,
      nrow = 4,
      scales = "free",
    ...
    theme(
      ...
      axis.text = element_text(family = "Avenir Next", size = 5),
    ...
    NULL -> p.linear_fit.by_gene
p.linear_fit.by_gene

ggsave(
  p.linear_fit.by_gene,
  filename = paste0(PARAM$folder.base, "results/linear_fits.by_gene.pdf"),
  width = 250,
  height = 250,
  unit = "mm",
  scale = 1,
